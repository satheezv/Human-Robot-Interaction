...
def step_wr(next_state):
    reward=0
    action(next_state[0],next_state[1])
    return reward, next_state
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown(): 
        ...
        #box_state="state_"+str(dis_from_goal_state.index(min(dis_from_goal_state))+1)
        #send robot to that state
        print("Current state state_"+str(dis_from_state.index(min(dis_from_state))+1))
        #print("Box state state_"+str(dis_from_goal_state.index(min(dis_from_goal_state))+1))
        #step(state)
        print("Actions:",action_space[state])
        action_sequence=action_sequence_from_RL
        for i in range(len(action_sequence)):
            actions=action_space[state]
            next_state=actions[action_sequence[i]]
            print(next_state)
            if next_state is not None:
                if i < len(action_sequence)-1:
                    ...
                    print("/"+next_state)
                    trans,rot = get_pose('/map', "/"+next_state)
                    b_trans,b_rot = get_pose("/"+next_state, '/Box_state')
                    if b_trans[0] == 0:
                       b_trans[0] = 0.01
                    if b_trans[1] == 0:
                       b_trans[1] = 0.01
